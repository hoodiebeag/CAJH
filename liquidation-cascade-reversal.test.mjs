...
test("classifies a liquidation-fetch failure precisely instead of silently dropping the asset", async () => {
  const report = await runLiquidationCascadeReversal({
    watchlist: ["XBT"],
...
test("classifies liquidation history shorter than minHistoryDays precisely, reporting the actual coverage", async () => {
...
test("reports a FAIL verdict (not a crash) when zero liquidation volume ever spikes above trailing average", async () => {
...
test("scores all three candidate multipliers on train, and only computes holdout when one produced a train trade", async () => {
