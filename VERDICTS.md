...
**Bottom line as of 2026-08-07: Tracks 1–4 of the tournament program have all failed or been abandoned, and low-vol/low-beta (B4) is now also killed on complete evidence.** Every price-structure entry family tested to date is net-negative or worse after costs. The open questions are PWR4 (classifier, blocked only by a prior data gate, not yet actually run), T1B-BREAKOUT-COSTFIX, T5 (exit-timing, not entry-signal), and T6 (the one untested timeframe axis). Nothing here should be read as "close to working" — it should be read as "these specific doors are shut, these specific ones are still open."
